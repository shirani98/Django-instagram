...
from api.serializers import CommentSerializer, FollowSerializer, LikeSerializer, PostDetailSerializer, PostListSerializer, PostCreateUpdateSerializer, UserDetailSerializer, UserListSerializer, UserUpdateSerializer
...
class DisLikePost(DestroyAPIView):
    ...
    def get_object(self):
        obj = Like.objects.filter(
            user=self.request.user.id, post=self.kwargs['post'])
        return obj
...
class FollowUser(CreateAPIView):
    ...
    def perform_create(self, serializer):
        user_for_follow = get_object_or_404(
            MyUser, username=self.kwargs['to_user'])
